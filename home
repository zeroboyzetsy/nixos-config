# /etc/nixos/home.nix
# Пользовательский уровень: Hyprland, пакеты, окружение
{ config, pkgs, lib, ... }:

{
  home.username = "ВАШ_ЛОГИН";         # ← замени
  home.homeDirectory = "/home/ВАШ_ЛОГИН"; # ← замени
  ...
  home.packages = with pkgs; [
    ...
    # браузер (если хочешь LibreWolf вместо Helium):
    # librewolf
  ];

  # ─── HYPRLAND КОНФИГ ───────────────────────────────────────────────────────
  # NixOS-модуль Home Manager для Hyprland генерирует hyprland.conf
  # (не .lua — Home Manager пока работает с классическим форматом, это нормально)
  wayland.windowManager.hyprland = {
    ...
    settings = {
      ...
      # Медиа-клавиши (работают на экране блокировки через bindl)
      bindl = [
